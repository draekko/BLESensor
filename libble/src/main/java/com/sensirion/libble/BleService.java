...
public class BleService extends Service implements ActionFailureCallback {
    ...
    public static final String EXTRA_DESCRIPTOR_UUID =
            "com.sensirion.libble.EXTRA_CHARACTERISTIC_UUID";
    private static final int MIN_SCAN_DURATION_MS = 10000;
    ...
    public boolean startScan(@NonNull final BleScanCallback callback, final long durationMs,
        ...
        if (false) {
            filters = getScanFilters(deviceNameFilter, advertisedServiceUuidFilter);
        } else {
    ...
    // Private Helpers
    @NonNull
    private List<ScanFilter> getScanFilters(final String[] deviceNameFilterList,
                                            final String[] serviceUuidFilterList) {
        final List<ScanFilter> filters = new ArrayList<>();
        if (deviceNameFilterList != null) {
            for (String deviceNameFilter : deviceNameFilterList) {
        ...
        if (serviceUuidFilterList != null) {
            for (String serviceUuidFilter : serviceUuidFilterList) {
                UUID uuid = UUID.fromString(serviceUuidFilter);
                ScanFilter filter = new ScanFilter.Builder()
